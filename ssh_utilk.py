...
class SSH_Class:
    ...
    def __init__(self, hostIP='', TCP_Port=22, userName='', passWord='', keyFilePath='', keyFilePassword='', timeOut=60,
                 commands=[]):
        self.hostIP = hostIP
    ...
    def connectByKey(self):
        ...
        ssh_Object.connect(hostname=self.hostIP, port=self.TCP_Port, username=self.userName, pkey=RSA_Key,
    ...
    def connectByPassword(self):
        ...
        ssh_Object = self.getSshObject()
        ssh_Object.connect(hostname=self.hostIP, port=self.TCP_Port, username=self.userName, password=self.passWord,
                           timeout=self.timeOut)
        if ssh_Object:
            return ssh_Object
        else:
